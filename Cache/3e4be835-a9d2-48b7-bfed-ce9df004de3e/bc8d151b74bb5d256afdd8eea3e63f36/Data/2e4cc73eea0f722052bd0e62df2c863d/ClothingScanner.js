...
let ClothingScanner = (() => {
    ...
    var ClothingScanner = _classThis = class extends _classSuper {
        ...
        async captureImage() {
            ...
            if (stillTexture) {
                ...
                try {
                    ...
                    print('ClothingScanner: Edit PNG (' + Math.round((this.lastCapturedBase64Edit.length * 0.75) / 1024) + ' KB)');
                }
                catch (editErr) {
        ...
        textureToBase64ForEdit(texture) {
            return new Promise((resolve, reject) => {
                try {
                    Base64.encodeTextureAsync(texture, (base64String) => resolve(base64String), () => reject(new Error('Failed to encode texture for edit')), CompressionQuality.LowQuality, EncodingType.Png);
                }
                catch (error) {
                    reject(error);
                }
...
- Only if no wearable wardrobe item is visible at all (an empty scene or an unrelated object), return clothing_found false and brand_model "NO_CLOTHING".`;
            ...
            try {
                ...
                if (parsed.clothing_found === false ||
                ...
                if (parsed.items && parsed.items.length > 0) {
                    for (let i = 0; i < parsed.items.length; i++) {
                        ...
                        it.top_speed = (0, ClosetTypes_1.clampStat)(it.top_speed);
                        it.acceleration = (0, ClosetTypes_1.clampStat)(it.acceleration);
                        it.braking = (0, ClosetTypes_1.clampStat)(it.braking);
                        it.traction = (0, ClosetTypes_1.clampStat)(it.traction);
                        it.comfort = (0, ClosetTypes_1.clampStat)(it.comfort);
                        it.rarity = (0, ClosetTypes_1.clampStat)(it.rarity || 2);
                        it.rarity_label = (0, ClosetTypes_1.getRarityLabel)(it.rarity);
