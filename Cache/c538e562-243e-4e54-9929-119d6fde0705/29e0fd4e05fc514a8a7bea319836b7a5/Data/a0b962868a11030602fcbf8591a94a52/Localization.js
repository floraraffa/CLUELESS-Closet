...
exports.translateStaticTextForObject = translateStaticTextForObject;
exports.getIdlePhrases = getIdlePhrases;
exports.getCheatMessages = getCheatMessages;
let currentLang = 'en';
...
const S = {
    ...
    language_welcome_heading: { en: 'Welcome to Closet Club', fr: 'Bienvenue a Closet Club', es: 'Bienvenido a Closet Club' },
    // -----------------------------------------------------------------
    // Welcome Container
    // -----------------------------------------------------------------
    welcome_title: { en: 'Look at a garment or outfit,\nor just scan you in front of a mirror', fr: 'Regardez une piece ou un look,\nou scannez-vous devant un miroir', es: 'Mira una prenda o un outfit,\no escaneate frente a un espejo' },
    welcome_description: {
        en: 'Scan garments and outfits with Spectacles.\n\nCloset Club reads each item, builds a smart closet, and turns every save into a polaroid-style wardrobe card.',
        fr: 'Scannez des vetements et des looks avec Spectacles.\n\nCloset Club lit chaque piece, construit un dressing intelligent et transforme chaque sauvegarde en carte de garde-robe style polaroid.',
        es: 'Escanea prendas y looks con Spectacles.\n\nCloset Club lee cada prenda, crea un armario inteligente y convierte cada guardado en una card de armario estilo polaroid.',
    },
    ...
    streak_long: { en: '{days}-day style streak · x{mult} bonus', fr: 'Série style de {days} jours · bonus x{mult}', es: 'Racha de estilo de {days} dias · bonus x{mult}' },
    streak_short: { en: '2-day style streak · x{mult} bonus', fr: 'Série style de 2 jours · bonus x{mult}', es: 'Racha de estilo de 2 dias · bonus x{mult}' },
    no_streak: { en: 'Daily style bonus ready', fr: 'Bonus style du jour prêt', es: 'Bonus de estilo diario listo' },
    ...
    trust_popup: { en: '+{amount}% Style Score', fr: '+{amount}% Score style', es: '+{amount}% Puntaje de estilo' },
    trust_penalty: { en: '-{amount}% Style Score', fr: '-{amount}% Score style', es: '-{amount}% Puntaje de estilo' },
    ...
    streak_bonus_long: { en: 'Welcome, {name}\n{days}-day style streak · x{mult} bonus', fr: 'Bienvenue, {name}\nSérie style de {days} jours · bonus x{mult}', es: 'Bienvenido, {name}\nRacha de estilo de {days} dias · bonus x{mult}' },
    streak_bonus_short: { en: 'Welcome, {name}\n2-day style streak · x{mult} bonus', fr: 'Bienvenue, {name}\nSérie style de 2 jours · bonus x{mult}', es: 'Bienvenido, {name}\nRacha de estilo de 2 dias · bonus x{mult}' },
    welcome_user: { en: 'Welcome, {name}', fr: 'Bienvenue, {name}', es: 'Bienvenido, {name}' },
    daily_bonus: { en: '{days}-day style streak · x{mult} bonus', fr: 'Série style de {days} jours · bonus x{mult}', es: 'Racha de estilo de {days} dias · bonus x{mult}' },
    daily_bonus_none: { en: 'Daily style bonus ready', fr: 'Bonus style du jour prêt', es: 'Bonus de estilo diario listo' },
    ...
    xp_placeholder: { en: 'Daily style bonus', fr: 'Bonus style du jour', es: 'Bonus de estilo diario' },
    style_score_static: { en: 'Style Icon | Style Score: 100%', fr: 'Icône style | Score style : 100%', es: 'Icono de estilo | Puntaje de estilo: 100%' },
    level_placeholder: { en: 'Style level ready', fr: 'Niveau style prêt', es: 'Nivel de estilo listo' },
    look_analysis_label: { en: 'LOOK ANALYSIS', fr: 'ANALYSE DU LOOK', es: 'ANALISIS DEL LOOK' },
    match_label: { en: 'MATCH', fr: 'MATCH', es: 'COMBINA' },
    ...
    scanned_by_preview: { en: 'Styled by: Username\nStyle Score: 100%', fr: 'Stylé par : Username\nScore style : 100%', es: 'Estilizado por: Username\nPuntaje de estilo: 100%' },
    // -----------------------------------------------------------------
    // Cloud Manager
    // -----------------------------------------------------------------
...
const CHEAT_MESSAGES_TABLE = {
    ...
    es: [
        ...
        "El Puntaje de estilo baja otra vez.\nEscanea prendas reales.",
    ],
};
// =====================================================================
// PUBLIC API
...
const STATIC_OBJECT_NAME_TO_KEY = {
...
function translateStaticTextForObject(objectName, text) {
    const key = STATIC_OBJECT_NAME_TO_KEY[normalizeStaticText(objectName)];
    if (key)
        return t(key);
    return translateStaticText(text);
}
function getIdlePhrases() {
    return IDLE_PHRASES_TABLE[currentLang] || IDLE_PHRASES_TABLE['en'];
}
function getCheatMessages() {
