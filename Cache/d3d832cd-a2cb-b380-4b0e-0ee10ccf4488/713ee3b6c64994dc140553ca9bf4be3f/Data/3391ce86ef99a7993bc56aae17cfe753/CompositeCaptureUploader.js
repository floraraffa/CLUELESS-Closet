...
exports.CompositeCaptureUploader = void 0;
var __selfType = requireType("./CompositeCaptureUploader");
...
let CompositeCaptureUploader = (() => {
    let _classDecorators = [component];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = BaseScriptComponent;
    var CompositeCaptureUploader = _classThis = class extends _classSuper {
        constructor() {
            super();
            ...
            this.videoStorageFolder = this.videoStorageFolder;
            this.audioStorageFolder = this.audioStorageFolder;
            this.stitchedOutputFolder = this.stitchedOutputFolder;
            this.shareToSpotlight = this.shareToSpotlight;
            this.captionInput = this.captionInput;
            this.defaultCaption = this.defaultCaption;
            this.useVerticalCrop = this.useVerticalCrop;
            this.verticalCropSwitch = this.verticalCropSwitch;
            this.imageFormat = this.imageFormat;
            this.frameQuality = this.frameQuality;
            this.frameRate = this.frameRate;
            this.maxRecordingDuration = this.maxRecordingDuration;
            ...
            this.recordingEndTime = 0; // Captured when recording stops (before uploads)
            this.actualRecordingDurationMs = 0; // True recording duration
            // Frame capture state - stored in memory during recording, uploaded after
            this.frameCount = 0;
            this.lastFrameTime = 0;
            this.storedFrames = [];
            this.isCapturingFrame = false; // Lock to prevent overlapping captures
         ...
         */
        async initializeCompositeCapture() {
        ...
        async startRecording() {
            ...
            this.sessionId = CaptureUtilities_1.SessionUtility.generateSessionId("composite");
        ...
        async stopRecording() {
            ...
            for (let i = 0; i < this.storedFrames.length; i++) {
                ...
                const success = await this.videoCapture.uploadFrameToStorage(this.supabaseAuth.getSupabaseClient(), this.storageBucket, this.sessionId, frame.frameNumber, frame.frameData, frame.timestamp, this.videoStorageFolder, this.imageFormat // Pass image format (0 = JPG, 1 = PNG)
                );
                if (!success) {
            ...
            this.log(`Composite recording complete: ${totalFrames} frames, ${totalAudio} audio chunks in ${durationSec.toFixed(1)}s`);
            this.updateStatus(`✅ Complete! ${totalFrames} frames + ${totalAudio} audio (${durationSec.toFixed(1)}s) - Stitching...`);
        }
        /**
         * Update final metadata with recording statistics
         * Returns true if upload was successful, false otherwise
         */
        async updateFinalMetadata() {
            try {
        ...
        async verifyMetadataExists() {
            try {
                ...
                if (hasFiles) {
                    this.log(`Metadata files: ${data.map((f) => f.name).join(", ")}`);
                }
                return hasFiles;
            }
            catch (error) {
         ...
         */
        async captureCompositeFrame(texture) {
            return new Promise((resolve, reject) => {
                try {
                    // Use quality setting for compression
                    const compressionQuality = this.frameQuality > 70 ? CompressionQuality.HighQuality : CompressionQuality.IntermediateQuality;
                    ...
                    }, compressionQuality, encodingType);
                }
                catch (error) {
                    reject(error);
                }
         ...
         */
        async uploadAudioChunk(sessionId, chunkNumber, wavData) {
            try {
    ...
    __setFunctionName(_classThis, "CompositeCaptureUploader");
    (() => {
        const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
        __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
        CompositeCaptureUploader = _classThis = _classDescriptor.value;
        if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
        __runInitializers(_classThis, _classExtraInitializers);
    })();
    return CompositeCaptureUploader = _classThis;
})();
exports.CompositeCaptureUploader = CompositeCaptureUploader;
//# sourceMappingURL=CompositeCaptureUploader.js.map