...
exports.VehicleCardUI = void 0;
var __selfType = requireType("./VehicleCardUI");
...
let VehicleCardUI = (() => {
    ...
    var VehicleCardUI = _classThis = class extends _classSuper {
        constructor() {
            super();
            // =====================================================================
        ...
        displayResults(data) {
            ...
            if (this.vehicleCard) {
                ...
                (0, VehicleTypes_1.enableAllDescendants)(this.vehicleCard);
                this.hideScanResultReviewButton();
            }
            ...
            if (this.carTypeText)
                this.carTypeText.text = (0, VehicleTypes_1.formatCarType)(data.category || data.type);
            if (this.carYearText)
                this.carYearText.text = detailParts.length > 0 ? detailParts.join(' / ') : (data.subcategory || data.year || '');
            // Rarity display (★★★☆☆ Uncommon)
            const rarityDisplay = (0, VehicleTypes_1.formatRarityText)(data.rarity, data.rarity_label);
            if (this.carRarityText) {
                this.carRarityText.text = rarityDisplay;
            }
            else if (this.vehicleCard) {
                const rarityObj = (0, VehicleTypes_1.findChildByName)(this.vehicleCard, 'Car Rarity');
                if (rarityObj) {
            ...
            print('VehicleCardUI: ' + displayName + ' (' + (0, VehicleTypes_1.formatCarType)(data.category || data.type) + ') ' + (data.color || ''));
        }
        /**
        ...
        applyItemPhoto(texture) {
            ...
            try {
                ...
                print('VehicleCardUI: applyItemPhoto error: ' + e);
            }
        }
        ...
        connectButton(buttonObj, callback, debugName) {
            ...
            if (!connected) {
                print('VehicleCardUI: [' + debugName + '] no compatible button event found');
            }
            return connected;
        }
        connectButtonRecursive(buttonObj, callback, debugName, depth) {
            ...
            for (let i = 0; i < scripts.length; i++) {
                ...
                if (script.onButtonPinched && typeof script.onButtonPinched.add === 'function') {
                    ...
                    print('VehicleCardUI: [' + debugName + '] connected via onButtonPinched on ' + buttonObj.name);
                    return true;
                }
                // UIKit onTriggerUp
                if (script.onTriggerUp && typeof script.onTriggerUp.add === 'function') {
                    script.onTriggerUp.add(() => callback());
                    print('VehicleCardUI: [' + debugName + '] connected via onTriggerUp on ' + buttonObj.name);
                    return true;
                }
                // SIK onTriggerEnd
                if (script.onTriggerEnd && typeof script.onTriggerEnd.add === 'function') {
                    script.onTriggerEnd.add(() => callback());
                    print('VehicleCardUI: [' + debugName + '] connected via onTriggerEnd on ' + buttonObj.name);
                    return true;
                }
                // SIK onTriggerStart
                if (script.onTriggerStart && typeof script.onTriggerStart.add === 'function') {
                    script.onTriggerStart.add(() => callback());
                    print('VehicleCardUI: [' + debugName + '] connected via onTriggerStart on ' + buttonObj.name);
                    return true;
                }
                ...
                if (script.onStateChanged && typeof script.onStateChanged.add === 'function') {
                    ...
                    print('VehicleCardUI: [' + debugName + '] connected via onStateChanged on ' + buttonObj.name);
                    return true;
                }
                ...
                if (script.onInteractorTriggerEnd && typeof script.onInteractorTriggerEnd.add === 'function') {
                    ...
                    print('VehicleCardUI: [' + debugName + '] connected via onInteractorTriggerEnd on ' + buttonObj.name);
                    return true;
                }
            }
        ...
        startEditingNote() {
            const noteText = this.getStyleNotesText();
            if (!noteText) {
                print('VehicleCardUI: No Style Notes text found for editing');
                return;
            }
            try {
                if (!global.textInputSystem) {
                    print('VehicleCardUI: TextInputSystem unavailable on this device/preview');
                    return;
                }
                ...
                options.onError = (error, description) => {
                    print('VehicleCardUI: note keyboard error ' + error + ': ' + description);
                };
                global.textInputSystem.requestKeyboard(options);
            }
            catch (e) {
                print('VehicleCardUI: startEditingNote error: ' + e);
            }
        }
        buildStyleNote(data) {
            const parts = [];
        ...
        getStyleNotesText() {
            ...
            if (!this.vehicleCard)
                return null;
            const noteObj = (0, VehicleTypes_1.findChildByName)(this.vehicleCard, 'Style Notes') ||
                (0, VehicleTypes_1.findChildByName)(this.vehicleCard, 'Card Notes') ||
                (0, VehicleTypes_1.findChildByName)(this.vehicleCard, 'User Note');
            if (!noteObj)
                return null;
        ...
        setupNoteEditButton() {
            ...
            const button = this.editNoteButton ||
                (0, VehicleTypes_1.findChildByName)(this.vehicleCard, 'Edit Note Button') ||
                (0, VehicleTypes_1.findChildByName)(this.vehicleCard, 'Edit Notes Button') ||
                (0, VehicleTypes_1.findChildByName)(this.vehicleCard, 'Edit Note') ||
                (0, VehicleTypes_1.findChildByName)(this.vehicleCard, 'Edit Notes');
            if (!button)
                return;
        ...
        hideScanResultReviewButton() {
            if (!this.vehicleCard)
                return;
            const reviewButton = (0, VehicleTypes_1.findChildByName)(this.vehicleCard, 'Review Button');
            if (reviewButton)
        ...
        setupCloseButton() {
            ...
            pollEvent.bind(() => {
                ...
                if (attempts % 30 === 0) {
                    const frameObj = (0, VehicleTypes_1.findChildByName)(this.vehicleCard, 'FrameObject');
    ...
    __setFunctionName(_classThis, "VehicleCardUI");
    (() => {
        const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
        __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
        VehicleCardUI = _classThis = _classDescriptor.value;
        if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
        __runInitializers(_classThis, _classExtraInitializers);
    })();
    return VehicleCardUI = _classThis;
})();
exports.VehicleCardUI = VehicleCardUI;
//# sourceMappingURL=VehicleCardUI.js.map