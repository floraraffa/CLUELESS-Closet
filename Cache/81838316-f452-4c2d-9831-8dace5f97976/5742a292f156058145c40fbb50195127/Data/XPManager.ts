...
export class XPManager extends BaseScriptComponent {
    ...
    rewardTrust(): void {
        ...
        if (this.profile.trustScore < 100) {
            ...
            print('XPManager: Style Score reward +' + TRUST_SCAN_REWARD + ' → ' + this.profile.trustScore + '%');
    ...
    private buildWelcomeGreetingMessage(): string {
        ...
        const mult = getStreakMultiplier(days);
        if (days >= 3) {
            return tf('streak_bonus_long', { name: this.getDisplayName(), days: days, mult: mult.toFixed(1) });
        }
        if (days === 2) {
            return tf('streak_bonus_short', { name: this.getDisplayName(), mult: mult.toFixed(1) });
        }
        return tf('welcome_user', { name: this.getDisplayName() }) + '\n' + t('daily_bonus_none');
    }
    ...
    private refreshUserCardUI(): void {
        ...
        if (this.levelNumberText) {
            this.levelNumberText.text = tf('welcome_user', { name: this.getDisplayName() });
