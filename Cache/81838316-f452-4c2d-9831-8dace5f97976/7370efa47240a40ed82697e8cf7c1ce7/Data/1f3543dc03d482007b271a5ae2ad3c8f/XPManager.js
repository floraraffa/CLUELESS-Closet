...
let XPManager = (() => {
    ...
    var XPManager = _classThis = class extends _classSuper {
        constructor() {
            ...
            // INTERNAL STATE
            // =====================================================================
            this.profile = (0, VehicleTypes_1.createDefaultProfile)();
            ...
            // =====================================================================
            this.tradedSerials = new Set();
            this.tradeXPCount = 0;
            this.lastTradeTime = 0;
            this.TRADE_XP_AMOUNT = 15;
            this.GIVE_XP_AMOUNT = 25;
            this.TRADE_COOLDOWN_MS = 60000; // 60 seconds between trade XP
            this.MAX_TRADE_XP_PER_SESSION = 10;
            // =====================================================================
            // XP LOGIC
            // =====================================================================
         ...
         */
        awardScanXP(rarity) {
            const baseXP = VehicleTypes_1.XP_PER_RARITY[rarity] || VehicleTypes_1.XP_PER_RARITY[2];
            const multiplier = (0, VehicleTypes_1.getStreakMultiplier)(this.profile.streakDays);
        ...
        awardTradeXP(cardSerial) {
            ...
            if (cardSerial)
                this.tradedSerials.add(cardSerial);
            this.tradeXPCount++;
        ...
        getCurrentLevelDef() {
            return (0, VehicleTypes_1.getLevelForXP)(this.profile.totalXP);
        }
        ...
        addXP(amount) {
            ...
            const newLevelDef = (0, VehicleTypes_1.getLevelForXP)(this.profile.totalXP);
            ...
            if (this.profile.level > previousLevel) {
                for (let lvl = previousLevel + 1; lvl <= this.profile.level; lvl++) {
                    const idx = Math.min(lvl - 1, VehicleTypes_1.LEVEL_TABLE.length - 1);
                    const lvlDef = VehicleTypes_1.LEVEL_TABLE[idx];
        ...
        penalizeTrust() {
            const penalty = (0, VehicleTypes_1.getTrustPenalty)(this.profile.consecutiveCheats);
        ...
        rewardTrust() {
            ...
            if (this.profile.trustScore < 100) {
                this.profile.trustScore = Math.min(100, this.profile.trustScore + VehicleTypes_1.TRUST_SCAN_REWARD);
                print('XPManager: Trust reward +' + VehicleTypes_1.TRUST_SCAN_REWARD + ' → ' + this.profile.trustScore + '%');
        ...
        getTrustDisplayString() {
            const rank = (0, VehicleTypes_1.getTrustRank)(this.profile.trustScore);
            const name = this.cachedUsername.length > 0 ? this.cachedUsername : 'Driver';
            return name + '\n' + rank.name + ' | ' + (0, VehicleTypes_1.formatTrustText)(this.profile.trustScore);
        }
        /** Returns a compact trust string for the User Card Info. */
        getTrustCardString() {
            const rank = (0, VehicleTypes_1.getTrustRank)(this.profile.trustScore);
            return rank.name + '\n' + (0, VehicleTypes_1.formatTrustText)(this.profile.trustScore);
        }
        migrateOldTrust() {
            try {
        ...
        applyTrustColor(textComp) {
            try {
                const hex = (0, VehicleTypes_1.getTrustColor)(this.profile.trustScore);
        ...
        refreshUserCardUI() {
            ...
            const levelDef = (0, VehicleTypes_1.getLevelForXP)(this.profile.totalXP);
            const prestigeStr = (0, VehicleTypes_1.formatPrestigeStars)(this.profile.prestige);
            ...
            if (this.levelNumberText) {
                let label = (0, Localization_1.t)('lvl_prefix') + levelDef.level + ': ' + levelDef.name;
                if (prestigeStr.length > 0)
                    label = prestigeStr + ' ' + label;
            ...
            if (this.xpCounterText) {
                ...
                if (nextThreshold < 0) {
                    this.xpCounterText.text = (0, Localization_1.tf)('xp_max', { xp: this.profile.totalXP });
                }
                else {
                    this.xpCounterText.text = (0, Localization_1.tf)('xp_progress', { xp: this.profile.totalXP, next: nextThreshold });
                }
            ...
            if (this.bonusInfoText) {
                const mult = (0, VehicleTypes_1.getStreakMultiplier)(this.profile.streakDays);
                let bonusText = '';
                if (this.profile.streakDays >= 3) {
                    bonusText = (0, Localization_1.tf)('streak_long', { days: this.profile.streakDays, mult: mult.toFixed(1) });
                }
                else if (this.profile.streakDays === 2) {
                    bonusText = (0, Localization_1.tf)('streak_short', { mult: mult.toFixed(1) });
                }
                else {
                    bonusText = (0, Localization_1.t)('no_streak');
                }
                if (this.profile.prestige > 0) {
                    bonusText += '\n' + (0, VehicleTypes_1.formatPrestigeStars)(this.profile.prestige) + ' ' + (0, Localization_1.t)('prestige_label') + ' ' + this.profile.prestige;
                }
            ...
            if (this.userCardText) {
                this.userCardText.text = (0, Localization_1.t)('lvl_prefix') + levelDef.level;
            }
        }
        updateXPBar() {
            if (!this.xpBarParent)
        ...
        showWelcomeBonusPopup() {
            ...
            let message = '';
            if (this.profile.streakDays >= 3) {
                message = (0, Localization_1.tf)('streak_bonus_long', { days: this.profile.streakDays, mult: mult.toFixed(1) });
            }
            else {
                message = (0, Localization_1.tf)('streak_bonus_short', { mult: mult.toFixed(1) });
            }
        ...
        enqueueTrustPopup() {
            ...
            this.popupQueue.push({
                text: (0, Localization_1.tf)('trust_popup', { amount: VehicleTypes_1.TRUST_SCAN_REWARD }),
                r: r, g: g, b: b,
                hold: 2.5, drift: 1,
            });
            if (this.onPositiveProgress)
        ...
        loadProfile() {
            try {
                ...
                if (jsonStr && jsonStr.length > 0) {
                    ...
                    this.profile.trustScore = typeof parsed.trustScore === 'number' ? parsed.trustScore : VehicleTypes_1.TRUST_DEFAULT;
                    ...
                    const computed = (0, VehicleTypes_1.getLevelForXP)(this.profile.totalXP);
