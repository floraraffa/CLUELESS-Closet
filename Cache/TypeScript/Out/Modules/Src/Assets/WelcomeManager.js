...
let WelcomeManager = (() => {
    ...
    var WelcomeManager = _classThis = class extends _classSuper {
        ...
        applyLanguageTranslationsToScene() {
            try {
                const rootCount = global.scene.getRootObjectsCount();
                for (let i = 0; i < rootCount; i++) {
                    const root = global.scene.getRootObject(i);
                    if (root)
                        this.applyLanguageTranslationsRecursive(root, 0);
                }
            }
            catch (e) {
        ...
        applyLanguageTranslationsRecursive(obj, depth) {
            if (!obj || depth > 80)
                return;
            const textComp = obj.getComponent('Component.Text');
            if (textComp) {
                const translated = (0, Localization_1.translateStaticTextForObject)(obj.name, textComp.text);
            ...
            for (let i = 0; i < childCount; i++) {
                ...
                if (child)
                    this.applyLanguageTranslationsRecursive(child, depth + 1);
            }
        }
        // =====================================================================
        // BUTTON SETUP
