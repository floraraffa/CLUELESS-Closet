...
var __selfType = requireType("./XPManager");
...
let XPManager = (() => {
    ...
    var XPManager = _classThis = class extends _classSuper {
        constructor() {
            super();
            // =====================================================================
            // INPUTS — XP Container (above-head profile display)
            // =====================================================================
            this.xpContainer = this.xpContainer;
            this.levelNumberText = this.levelNumberText;
            this.xpCounterText = this.xpCounterText;
            this.xpBarParent = this.xpBarParent;
            this.levelBadge = this.levelBadge;
            this.levelBadgeUrlTemplate = this.levelBadgeUrlTemplate;
            this.levelBadgePadDigits = this.levelBadgePadDigits;
            this.levelUpBadgeReveal = this.levelUpBadgeReveal;
            this.levelUpTitleText = this.levelUpTitleText;
            this.levelUpBodyText = this.levelUpBodyText;
            this.levelUpRevealHoldDuration = this.levelUpRevealHoldDuration;
            // =====================================================================
            // INPUTS — XP Display (camera-attached floating popup)
            // =====================================================================
            this.xpDisplay = this.xpDisplay;
            // =====================================================================
            // INPUTS — User Card Info (hand-mounted profile card)
            // =====================================================================
            this.userCardInfo = this.userCardInfo;
            this.acknowledgeButton = this.acknowledgeButton;
            // =====================================================================
            // INPUTS — User Card Button (on hand)
            // =====================================================================
            this.userCardButton = this.userCardButton;
            this.prestigeButton = this.prestigeButton;
            this.userCardText = this.userCardText;
            // =====================================================================
            ...
            // INTERNAL STATE
            // =====================================================================
            this.profile = (0, ClosetTypes_1.createDefaultProfile)();
            this.isUserCardVisible = false;
            this.userCardButtonConnected = false;
            this.userCardChildrenResolved = false;
            this.prestigeButtonConnected = false;
            this.cachedUsername = '';
            // Trust score — stored inside profile (no separate key)
            // Auto-resolved from userCardInfo children
            this.bonusInfoText = null;
            this.usernameInfoText = null;
            this.trustInfoText = null;
            this.userCardLevelBadge = null;
            this.remoteMediaModule = require('LensStudio:RemoteMediaModule');
            this.internetModule = require('LensStudio:InternetModule');
            this.badgeTextureCache = new Map();
            this.badgeLoadingLevels = new Set();
            this.badgePendingCallbacks = new Map();
            this.lastAppliedBadgeLevel = -1;
            this.lastAppliedUserCardBadgeLevel = -1;
            this.levelUpRevealQueue = [];
            ...
            this.manropeExtraBoldFont = null;
            this.manropeExtraBoldResolved = false;
            // =====================================================================
            // XP LOGIC
            // =====================================================================
            this.MAX_XP = 10000000;
            // =====================================================================
            // XP POPUP ANIMATION — "+XX XP" floating text on camera
            // =====================================================================
            this.MAX_POPUP_QUEUE = 10;
            this._userCardCloseHooked = false;
            // =====================================================================
            // ACKNOWLEDGE BUTTON — Closes User Card Info
            // =====================================================================
            this.acknowledgeButtonConnected = false;
        }
        __initialize() {
            super.__initialize();
            // =====================================================================
        ...
        onAwake() {
            ...
            if (this.xpContainer)
                this.xpContainer.enabled = false;
         ...
         */
        awardScanXP(rarity) {
            const baseXP = ClosetTypes_1.XP_PER_RARITY[rarity] || ClosetTypes_1.XP_PER_RARITY[2];
            const multiplier = (0, ClosetTypes_1.getStreakMultiplier)(this.profile.streakDays);
        ...
        getCurrentLevelDef() {
            return (0, ClosetTypes_1.getLevelForXP)(this.profile.totalXP);
        }
        getLevelBadgeUrl(level) {
            if (!this.levelBadgeUrlTemplate || this.levelBadgeUrlTemplate.length < 8)
                return '';
        ...
        loadBadgeTexture(level, onReady) {
            const clamped = Math.max(1, Math.min(this.MAX_LEVEL, Math.round(level)));
            ...
            if (!url || !this.remoteMediaModule || !this.internetModule)
                return;
            ...
            if (this.badgeLoadingLevels.has(clamped))
                return;
        ...
        applyBadgeTextureToObject(target, tex) {
            if (!target)
                return;
            try {
        ...
        updateLocalLevelBadges(level) {
            if (level !== this.lastAppliedBadgeLevel) {
                this.lastAppliedBadgeLevel = level;
                this.loadBadgeTexture(level, (tex) => this.applyBadgeTextureToObject(this.levelBadge, tex));
            }
            if (this.userCardLevelBadge && level !== this.lastAppliedUserCardBadgeLevel) {
                this.lastAppliedUserCardBadgeLevel = level;
                this.loadBadgeTexture(level, (tex) => this.applyBadgeTextureToObject(this.userCardLevelBadge, tex));
            }
        }
        /**
        ...
        prestigeReset() {
            ...
            if (this.onLevelUp) {
                this.onLevelUp(1, (0, Localization_1.tf)('prestige_up', { n: this.profile.prestige }));
            }
        }
        /**
        ...
        toggleUserCard() {
            ...
            if (this.isUserCardVisible) {
                this.refreshUserCardUI();
                if (this.onUserCardOpened)
                    this.onUserCardOpened();
            }
            else {
                if (this.onUserCardClosed)
                    this.onUserCardClosed();
            }
        ...
        addXP(amount) {
            ...
            const newLevelDef = (0, ClosetTypes_1.getLevelForXP)(this.profile.totalXP);
            this.profile.level = newLevelDef.level;
            this.saveProfile();
            if (this.onCloudSyncProfile)
                this.onCloudSyncProfile(this.getProfile());
            ...
            if (this.profile.level > previousLevel) {
                for (let lvl = previousLevel + 1; lvl <= this.profile.level; lvl++) {
                    const idx = Math.min(lvl - 1, ClosetTypes_1.LEVEL_TABLE.length - 1);
                    const lvlDef = ClosetTypes_1.LEVEL_TABLE[idx];
                    print('XPManager: LEVEL UP! ' + (lvl - 1) + ' -> ' + lvl + ' (' + lvlDef.name + ')');
                    if (this.onLevelUp)
                        this.onLevelUp(lvl, lvlDef.name);
        ...
        enqueueLevelUpReveal(fromLevel, toLevel, toName) {
        ...
        showNextLevelUpReveal() {
            ...
            if (!this.levelUpBadgeReveal || !this.levelUpTitleText || !this.levelUpBodyText) {
                this.enqueueCustomPopup((0, Localization_1.tf)('level_up_reveal_body', { from: item.from, to: item.to, name: item.toName }), 1, 1, 1, 2.5, 1);
                this.showNextLevelUpReveal();
                return;
            }
            this.levelUpTitleText.text = (0, Localization_1.t)('level_up_reveal_title');
            this.levelUpBodyText.text = (0, Localization_1.tf)('level_up_reveal_body', { from: item.from, to: item.to, name: item.toName });
            ...
            this.levelUpBadgeReveal.getTransform().setLocalScale(new vec3(baseBadgeScale.x * 0.01, baseBadgeScale.y * 0.01, baseBadgeScale.z * 0.01));
            this.loadBadgeTexture(item.to, (tex) => {
                this.applyBadgeTextureToObject(this.levelUpBadgeReveal, tex);
            });
            if (this.levelUpRevealAnimEvent) {
                try {
                    this.levelUpRevealAnimEvent.enabled = false;
                }
                catch (e) { /* ignore */ }
                this.levelUpRevealAnimEvent = null;
            }
            ...
            this.levelUpRevealAnimEvent.bind(() => {
                ...
                if (e < IN) {
                    ...
                    this.levelUpBadgeReveal.getTransform().setLocalScale(new vec3(baseBadgeScale.x * mult, baseBadgeScale.y * mult, baseBadgeScale.z * mult));
        ...
        resolveLevelUpRevealInputs() {
            ...
            if (!this.levelUpTitleText) {
                ...
                if (titleObj) {
                    const textComp = titleObj.getComponent('Component.Text');
                    if (textComp) {
            ...
            if (!this.levelUpBodyText) {
                ...
                if (bodyObj) {
                    const textComp = bodyObj.getComponent('Component.Text');
                    if (textComp) {
        ...
        findSceneObjectByName(targetName) {
            try {
        ...
        findSceneObjectByNameRecursive(obj, targetLower) {
            if (obj.name && obj.name.toLowerCase() === targetLower)
                return obj;
        ...
        // =====================================================================
        getTrustScore() { return this.profile.trustScore; }
        getConsecutiveCheats() { return this.profile.consecutiveCheats; }
        /** Called when anti-cheat detects a cheat attempt. */
        penalizeTrust() {
            const penalty = (0, ClosetTypes_1.getTrustPenalty)(this.profile.consecutiveCheats);
        ...
        rewardTrust() {
            ...
            if (this.profile.trustScore < 100) {
                this.profile.trustScore = Math.min(100, this.profile.trustScore + ClosetTypes_1.TRUST_SCAN_REWARD);
                print('XPManager: Fashion Trust reward +' + ClosetTypes_1.TRUST_SCAN_REWARD + ' → ' + this.profile.trustScore + '%');
        ...
        getTrustDisplayString() {
            const rank = (0, ClosetTypes_1.getTrustRank)(this.profile.trustScore);
        ...
        getFashionTrustText() {
            return (0, Localization_1.tf)('fashion_trust_score', { score: Math.round(Math.max(0, Math.min(100, this.profile.trustScore))) });
        }
        getLocalizedTrustRankName(rankName) {
            switch (rankName) {
                case 'Style Icon': return (0, Localization_1.t)('rank_style_icon');
                case 'Trusted Stylist': return (0, Localization_1.t)('rank_trusted_stylist');
                case 'Closet Lover': return (0, Localization_1.t)('rank_closet_lover');
                case 'Style Explorer': return (0, Localization_1.t)('rank_style_explorer');
                case 'Needs Review': return (0, Localization_1.t)('rank_needs_review');
                case 'Scan Rookie': return (0, Localization_1.t)('rank_scan_rookie');
                case 'Closet Newcomer': return (0, Localization_1.t)('rank_closet_newcomer');
                case 'Unverified Stylist': return (0, Localization_1.t)('rank_unverified');
            }
            return rankName;
        }
        getDisplayName() {
        ...
        getDailyBonusText() {
            ...
            const mult = (0, ClosetTypes_1.getStreakMultiplier)(days);
            if (days >= 2) {
                return (0, Localization_1.tf)('daily_bonus', { days: days, mult: mult.toFixed(1) });
            }
            return (0, Localization_1.t)('daily_bonus_none');
        }
        buildWelcomeGreetingMessage() {
            return (0, Localization_1.tf)('welcome_back_user', { name: this.getDisplayName() });
        }
        migrateOldTrust() {
            try {
                ...
                if (json && json.length > 0) {
                    ...
                    if (typeof parsed.score === 'number')
                        this.profile.trustScore = parsed.score;
                    if (typeof parsed.cheats === 'number')
                        this.profile.consecutiveCheats = parsed.cheats;
         ...
         */
        resolveUserCardChildren() {
            if (this.userCardChildrenResolved || !this.userCardInfo)
                return;
        ...
        refreshUserCardUI() {
            ...
            const levelDef = (0, ClosetTypes_1.getLevelForXP)(this.profile.totalXP);
            ...
            if (this.levelNumberText) {
                this.levelNumberText.text = (0, Localization_1.tf)('welcome_back_user', { name: this.getDisplayName() });
            ...
            if (this.bonusInfoText) {
                let bonusText = (0, Localization_1.tf)('style_level_line', { level: levelDef.level, name: levelDef.name });
                if (this.profile.prestige > 0) {
                    bonusText += '\n' + (0, ClosetTypes_1.formatPrestigeStars)(this.profile.prestige) + ' ' + (0, Localization_1.t)('prestige_label') + ' ' + this.profile.prestige;
                }
            ...
            if (this.userCardText) {
                this.userCardText.text = (0, Localization_1.tf)('style_level_short', { level: levelDef.level });
        ...
        updateXPBar() {
            ...
            const nextThreshold = (0, ClosetTypes_1.getXPForNextLevel)(levelDef.level);
            let fillRatio = 1.0;
        ...
        showWelcomePopupAnimation(message) {
            if (!this.xpDisplay)
                return;
            const displayObj = this.xpDisplay.getSceneObject();
            if (!displayObj)
            ...
            if (this.welcomeBonusAnimEvent) {
                try {
                    this.welcomeBonusAnimEvent.enabled = false;
                }
                catch (e) { /* ignore */ }
                this.welcomeBonusAnimEvent = null;
            }
        ...
        applyWelcomeGreetingStyle(textComp) {
            if (!textComp)
                return;
            this.forceTextBlack(textComp);
        ...
        getManropeExtraBoldFont() {
            if (this.manropeExtraBoldResolved)
            ...
            for (let i = 0; i < paths.length; i++) {
                try {
                    this.manropeExtraBoldFont = requireAsset(paths[i]);
        ...
        enqueueXPPopup(amount) {
            if (this.popupQueue.length >= this.MAX_POPUP_QUEUE)
                return;
            this.popupQueue.push({ text: (0, Localization_1.tf)('xp_popup', { amount: amount }), r: 1, g: 1, b: 1 });
            if (this.onPositiveProgress)
                this.onPositiveProgress();
        ...
        enqueueTrustPopup() {
            if (this.popupQueue.length >= this.MAX_POPUP_QUEUE)
                return;
            const hex = (0, ClosetTypes_1.getTrustColor)(this.profile.trustScore);
            const r = parseInt(hex.substring(0, 2), 16) / 255;
            const g = parseInt(hex.substring(2, 4), 16) / 255;
            const b = parseInt(hex.substring(4, 6), 16) / 255;
            this.popupQueue.push({
                text: (0, Localization_1.tf)('trust_popup', { amount: ClosetTypes_1.TRUST_SCAN_REWARD }),
                r: r, g: g, b: b,
                hold: 2.5, drift: 1,
            });
            if (this.onPositiveProgress)
        ...
        enqueueTrustPenaltyPopup(penalty) {
            if (this.popupQueue.length >= this.MAX_POPUP_QUEUE)
                return;
            this.popupQueue.push({
                text: (0, Localization_1.tf)('trust_penalty', { amount: penalty }),
                r: 1, g: 0.15, b: 0.1,
                hold: 3.0, drift: -1,
            });
            if (this.onNegativeProgress)
                this.onNegativeProgress();
            if (!this.isShowingPopup) {
                this.showNextPopup();
            }
        }
         ...
         */
        enqueueCustomPopup(text, r, g, b, hold, drift) {
            if (this.popupQueue.length >= this.MAX_POPUP_QUEUE)
                return;
        ...
        showNextPopup() {
            ...
            try {
                this.xpDisplay.textFill.color = new vec4(entry.r, entry.g, entry.b, 1);
            }
            catch (e) { /* ignore */ }
            displayObj.enabled = true;
            ...
            if (this.popupAnimEvent) {
                try {
                    this.popupAnimEvent.enabled = false;
                }
                catch (e) { /* ignore */ }
                this.popupAnimEvent = null;
            }
        ...
        setTextOpacity(textComp, opacity) {
            try {
        ...
        setObjectOpacity(obj, opacity) {
            try {
                const imgComp = obj.getComponent('Component.Image');
            ...
            try {
                const meshComp = obj.getComponent('Component.RenderMeshVisual');
        ...
        setupUserCardButton() {
            ...
            pollEvent.bind(() => {
                if (this.userCardButtonConnected) {
                ...
                try {
                    const scripts = this.userCardButton.getComponents('Component.ScriptComponent');
        ...
        setupPrestigeButton() {
            if (this.prestigeButton)
                this.prestigeButton.enabled = false;
            if (!this.prestigeButton || this.prestigeButtonConnected)
                return;
            if (this.onConnectButton) {
        ...
        hookCloseButtonOnShow() {
            if (this._userCardCloseHooked || !this.userCardInfo)
                return;
            let attempts = 0;
            const poll = this.createEvent('UpdateEvent');
            poll.bind(() => {
                attempts++;
                if (this._userCardCloseHooked || attempts > 120) {
                    poll.enabled = false;
                    return;
                }
        ...
        deepSearchAndHookClose(root) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                if (child.name === 'FrameObject') {
                    if (this.searchCloseButtonIn(child))
                        return true;
                }
                if (this.deepSearchAndHookClose(child))
                    return true;
            }
            return false;
        }
        searchCloseButtonIn(parent) {
            const count = parent.getChildrenCount();
            for (let i = 0; i < count; i++) {
                const child = parent.getChild(i);
                if (!child)
        ...
        setupAcknowledgeButton() {
            ...
            const pollEvent = this.createEvent('UpdateEvent');
            pollEvent.bind(() => {
                if (this.acknowledgeButtonConnected) {
                    pollEvent.enabled = false;
                    return;
                }
                attempts++;
                ...
                try {
                    const scripts = this.acknowledgeButton.getComponents('Component.ScriptComponent');
                    for (let i = 0; i < scripts.length; i++) {
                        const script = scripts[i];
                        if (!script)
                            continue;
        ...
        loadProfile() {
            try {
                ...
                if (jsonStr && jsonStr.length > 0) {
                    ...
                    this.profile.trustScore = typeof parsed.trustScore === 'number' ? parsed.trustScore : ClosetTypes_1.TRUST_DEFAULT;
                    ...
                    const computed = (0, ClosetTypes_1.getLevelForXP)(this.profile.totalXP);
    ...
    __setFunctionName(_classThis, "XPManager");
    (() => {
        const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
        __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
        XPManager = _classThis = _classDescriptor.value;
        if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
        __runInitializers(_classThis, _classExtraInitializers);
    })();
    return XPManager = _classThis;
})();
