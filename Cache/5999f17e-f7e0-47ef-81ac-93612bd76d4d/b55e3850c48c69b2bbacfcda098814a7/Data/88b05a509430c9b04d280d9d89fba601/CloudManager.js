...
let CloudManager = (() => {
    ...
    var CloudManager = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.onLeaderboardReady = null;
            this.onTradeReceived = null;
            this.onTradeStatusChanged = null;
            /** Called when an async trade is fully completed — card data for the receiver to add locally. */
            this.onAsyncTradeCompleted = null;
            this.onShowMessage = null;
            this.onGetProfile = null;
            this.onGetCollection = null;
            this.onGetUsername = null;
            this.onGetAvatarUrl = null;
            this.onShareComplete = null;
            this.onShareSucceeded = null;
            this.onShareFailed = null;
            this.onGetLocalImage = null;
            // =====================================================================
            // CONSTANTS
            // =====================================================================
            ...
            this.TABLE_TRADES = 'trade_requests';
            this.TABLE_TRADE_HISTORY = 'trade_history';
            this.TABLE_GALLERIES = 'shared_galleries';
            this.GALLERY_BASE_URL = 'https://clueless-gallery-web.vercel.app/g/';
            // =====================================================================
            // INTERNAL STATE
            // =====================================================================
            this.client = null;
            ...
            for (const cv of cloudVehicles) {
                ...
                if (!localV) {
                    ...
                    const imported = {
                        ...
                        quality: cv.quality || '',
                        scan_context: (0, VehicleTypes_1.normalizeScanContext)(cv.scan_context),
                        color: cv.color || '',
        ...
        async fetchLeaderboard(limit = 10) {
            ...
            try {
                ...
                if (this.onLeaderboardReady)
                    this.onLeaderboardReady(entries, myRank);
                return entries;
            }
            catch (e) {
        ...
        // ASYNC TRADING
        // =====================================================================
        async proposeTrade(toUserId, offeredSerial, offeredBrandModel, offeredRarity, message = '') {
            if (!this.isReady())
                return false;
            try {
                const tradeData = {
        ...
        async respondToTrade(tradeId, accept) {
            ...
            try {
                ...
                if (this.onTradeStatusChanged)
                    this.onTradeStatusChanged(tradeId, newStatus);
                if (accept) {
         ...
         */
        async executeAsyncTradeTransfer(tradeId) {
            try {
                ...
                const fromUser = tradeData.from_user;
                const serial = tradeData.offered_serial;
                if (!fromUser || !serial) {
                ...
                if (vErr || !vehicleData) {
                    ...
                    if (this.onShowMessage)
                        this.onShowMessage((0, Localization_1.t)('trade_failed_card'));
                    return;
                }
                ...
                if (this.onShowMessage)
                    this.onShowMessage((0, Localization_1.t)('trade_complete'));
                // 6. Notify locally
                if (this.onAsyncTradeCompleted) {
                    this.onAsyncTradeCompleted(vehicleData);
                }
            }
            catch (e) {
                print('CloudManager: Trade transfer exception: ' + e);
                if (this.onShowMessage)
                    this.onShowMessage((0, Localization_1.t)('trade_transfer_failed'));
            }
        }
        ...
        async syncTradeHistoryEntry(entry) {
            if (!this.isReady())
                return false;
            try {
                const row = {
            ...
            try {
                ...
                this.tradeChannel.on('postgres_changes', {
                    ...
                    if (payload?.new && this.onTradeReceived) {
                        this.onTradeReceived(payload.new);
                    }
                    if (this.onShowMessage) {
                        const trade = payload?.new;
                        this.onShowMessage((0, Localization_1.tf)('trade_request', { name: trade?.offered_brand_model || '?' }));
                    }
                });
                this.tradeChannel.on('postgres_changes', {
                    event: 'UPDATE',
                    ...
                    if (payload?.new) {
                        ...
                        if (this.onShowMessage && trade.status === 'accepted') {
                            this.onShowMessage((0, Localization_1.tf)('trade_accepted', { name: trade.offered_brand_model || '' }));
                        }
                    }
                });
                this.tradeChannel.subscribe((status) => {
        ...
        async searchPlayers(query, limit = 10) {
            ...
            if (sanitized.length < 2)
                return [];
            const escaped = sanitized.replace(/[%_\\]/g, (ch) => '\\' + ch);
            try {
                const { data, error } = await this.client
                    .from(this.TABLE_PROFILES)
                    .select('*')
        ...
        async fetchPlayerCollection(targetUserId) {
            if (!this.isReady())
                return [];
            try {
                const { data, error } = await this.client
        ...
        async shareCollection() {
            if (!this.isReady()) {
                ...
                if (this.onShowMessage)
                    this.onShowMessage((0, Localization_1.t)('not_connected'));
                if (this.onShareFailed)
                    this.onShareFailed();
                return '';
            }
            try {
                ...
                if (vehicles.length === 0) {
                    ...
                    if (this.onShowMessage)
                        this.onShowMessage((0, Localization_1.t)('scan_vehicles_first'));
                    if (this.onShareFailed)
                        this.onShareFailed();
                    return '';
                }
                const levelDef = (0, VehicleTypes_1.getLevelForXP)(profile.totalXP);
                ...
                const vehiclesSnapshot = vehicles.map((v) => ({
                    serial: v.serial,
                    brand: v.brand || '',
                ...
                for (let i = 0; i < vehiclesSnapshot.length; i++) {
                    ...
                    const match = cloudVehicles.find((cv) => cv.serial === serial);
                ...
                if (error) {
                    ...
                    if (this.onShowMessage)
                        this.onShowMessage((0, Localization_1.t)('share_failed'));
                    if (this.onShareFailed)
                        this.onShareFailed();
                    return '';
                }
                ...
                if (this.onShowMessage) {
                    this.onShowMessage((0, Localization_1.tf)('collection_shared_n', { count: vehiclesSnapshot.length }));
                }
