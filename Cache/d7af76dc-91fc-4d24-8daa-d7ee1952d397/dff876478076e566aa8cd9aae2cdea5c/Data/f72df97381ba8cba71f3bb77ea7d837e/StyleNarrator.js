    ...
    var StyleNarrator = _classThis = class extends _classSuper {
        ...
        async triggerOutfitFeedback(outfitItems, slotLabels, outfitText, percentText, matchPercent) {
            ...
            try {
                ...
                if (outfitText)
                    await this.speakDescriptionToExternalText(suggestion, outfitText);
                else
                    await this.speakDescription(suggestion);
            }
            catch (error) {
                ...
                if (outfitText)
                    await this.speakDescriptionToExternalText(fallback, outfitText);
                else
                    await this.speakDescription(fallback);
            }
            finally {
        ...
        async speakDescriptionToExternalText(text, targetText) {
            if (!targetText) {
...
${langInstruction}`;
            ...
            if (!this.ttsAudioPlayer) {
                ...
                if (externalTarget) {
                    ...
                    doneEv.bind(() => {
                        ...
                        if (this.speechTextTarget === externalTarget)
                            this.speechTextTarget = null;
                    });
                    doneEv.reset(15.0);
                }
        ...
        scheduleNextScroll() {
            if (this.scrollIndex >= this.scrollSentences.length - 1) {
                if (!this.ttsIsPlaying && !this.speechTextTarget)
