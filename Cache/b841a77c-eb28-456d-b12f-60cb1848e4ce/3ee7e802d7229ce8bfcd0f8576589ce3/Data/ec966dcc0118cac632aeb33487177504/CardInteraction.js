...
let CardInteraction = (() => {
    ...
    var CardInteraction = _classThis = class extends _classSuper {
        constructor() {
            ...
            this._moveFrameCounter = 0;
            this.MOVE_BROADCAST_INTERVAL = 3; // send position every N frames while dragging
        ...
        onCardTranslationEnd(cardIndex) {
            ...
            if (distToCarousel <= this.NEAR_CAROUSEL_DIST) {
                ...
                if (serial) {
                    ...
                    if (this.onCardPlacedInWorld)
                        this.onCardPlacedInWorld(serial, wPos, wRot, wScale);
                }
            }
            this.grabbedCardIndex = -1;
            if (this.onUpdateDeleteButtonVisibility)
