...
let CollectionManager = (() => {
    ...
    var CollectionManager = _classThis = class extends _classSuper {
        ...
        onComposeOutfitPressed() {
            ...
            // Let the AI compose from the whole closet; falls back to the local scorer if it fails.
            this.composeOutfitWithAI();
        }
        /** Local deterministic outfit composition — best-scoring item per slot. */
        composeOutfitDeterministic() {
            this.resetOutfitTesterSlots();
        ...
        async composeOutfitWithAI() {
            ...
            const sys = 'You are Closet Club, a fashion stylist. From the user\'s wardrobe below, compose the single best cohesive outfit. '
                + 'Pick at most ONE item per role, by its numeric index: top, layer (outerwear/jacket — optional), bottom, shoes, accessory (optional). '
                + 'Favor color harmony, matching style and occasion, and a complete look (a top or dress, plus a bottom and shoes). '
            ...
            try {
                ...
                const place = (idx, slot) => {
                    ...
                    if (!isNaN(n) && n >= 0 && n < items.length) {
                        this.assignSavedItemToOutfitSlot(slot, n, false);
                        count++;
                    }
                };
                ...
                place(pick.layer, 4);
                if (count === 0) {
                    print('CollectionManager: [OUTFIT-AI] AI returned no usable picks — using deterministic');
                    this.composeOutfitDeterministic();
                    return;
                }
                this.showOutfitBuilderContainer();
        ...
        getOutfitSlotAliases(slotIndex) {
            ...
            if (slotIndex === 4)
                return ['photocard TOP 2', 'PhotoCard TOP 2', 'Photocard TOP 2', 'Photo Card TOP 2', 'photocard TOP2', 'photocard LAYER', 'photocard OUTERWEAR', 'Garment LAYER', 'TOP 2', 'TOP2', 'Layer', 'Outerwear', 'Jacket'];
            return [];
        }
