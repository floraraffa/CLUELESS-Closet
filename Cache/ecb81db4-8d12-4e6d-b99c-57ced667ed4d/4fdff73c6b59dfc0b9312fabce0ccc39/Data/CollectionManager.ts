...
export class CollectionManager extends BaseScriptComponent {
    ...
    private async saveFullLookAsCards(): Promise<void> {
        ...
        for (let i = 0; i < cardObjs.length; i++) {
            ...
            if (showStatus) showStatus(tf('outfit_card_progress', { index: i + 1, total: cardObjs.length, name: name }));
            ...
            if (fullTex) this.applyCardImage(cardObjs[i], fullTex);
            ...
            try {
                ...
                this.saveGarmentCutoutTextureToStorage(savedList[i].savedAt, cutout, (saved: boolean, b64?: string) => {
                    const idx = this.findSavedVehicleIndexBySerial(savedList[i].serial);
                    if (idx < 0) return;
                    this.savedVehicles[idx].imageGenerated = saved;
                    this.saveCollectionToStorage();
                    if (saved && b64 && this.onCloudUploadImage) this.onCloudUploadImage(savedList[i].serial, b64);
                });
            } catch (e) {
                ...
                flyEv.bind(() => {
                    ...
                    if (t >= 1.0) {
                        ...
                        if (remaining <= 0) {
                            this.isSavingCard = false;
                            this.updateCollectionButtonLabel();
                            if (this.onShowDescription) this.onShowDescription(t === 1.0 ? t('card_collected') : '');
                            if (hideStatus) hideStatus(2.5);
                        }
                    }
                });
