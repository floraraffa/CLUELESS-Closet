...
export class CollectionManager extends BaseScriptComponent {
    ...
    private readonly SCAN_IMAGE_DIRECT_SAVE_MAX_CHARS: number = 650000;
    ...
    private onDeleteCardButtonPressed(): void {
        ...
        this.openDeleteConfirmForCard(targetIdx, 'handDeleteButton');
    }

    private openDeleteConfirmForCard(targetIdx: number, source: string): void {
        if (targetIdx < 0 || targetIdx >= this.savedVehicles.length || targetIdx >= this.collectionCardObjects.length) {
            if (this.onShowDescription) this.onShowDescription(t('no_card_delete'));
            if (this.onHideDescriptionAfterDelay) this.onHideDescriptionAfterDelay(2.0);
            return;
        }
        ...
        if (this.onShowDescription) {
            this.onShowDescription(tf('delete_confirm', { name: this.savedVehicles[targetIdx]?.brand_model || '?' }));
        }
        print('CollectionManager: [DELETE] Delete confirm opened from ' + source + ' for card #' + targetIdx);
    }
    ...
    private populateCollectorCard(cardObj: SceneObject, data: SavedVehicleData): void {
        ...
        this.setButtonLabel(cardObj, 'Review Button', t('delete_item_button'));
    ...
    private setOptionalCardText(cardObj: SceneObject, childName: string, text: string): void {
        ...
        if (tc) {
            tc.text = text;
            if (childName === 'Car Review') this.forceTextWhite(tc);
            else this.forceTextBlack(tc);
        }
    }
    ...
    private forceTextWhite(textComp: Text | null): void {
        if (!textComp) return;
        try {
    ...
    // =====================================================================
    // CARD DELETE BUTTON HOOKING — Deferred until collection is opened
    // =====================================================================

    /**
     * Hooks the card-level "Delete Item" action on all collector cards that
     * haven't been hooked yet. The object may still be named "Review Button"
     * in older prefabs, but its visible/user action is delete.
    ...
    private canTriggerCardDelete(cardIndex: number): boolean {
        const state = this.cardStates[cardIndex] || this.STATE_IN_COLLECTION;
        return state === this.STATE_IN_COLLECTION
            || state === this.STATE_PICKED
            || state === this.STATE_PLACED_IN_WORLD;
    }

    private triggerCardDeleteFromVertical(cardIndex: number, data: SavedVehicleData, source: string): void {
        if (!this.canTriggerCardDelete(cardIndex)) {
            print('CollectionManager: [DELETE] Ignored (' + source + ') - card is not available');
            return;
        }

        this.cancelCardGrabForButtonCombine(cardIndex);
        this.openDeleteConfirmForCard(cardIndex, source);
    }
    ...
    /**
     * Attempts to connect the Delete Item action on a specific collector card.
     * Older prefabs may still name the object "Review Button"; the visible
     * label is set to Delete Item and this button opens the normal delete flow.
     */
    private tryHookReviewButton(cardObj: SceneObject, cardIndex: number): boolean {
        ...
        if (!reviewBtnObj) {
            print('CollectionManager: [DELETE] Delete Item button not found in card #' + cardIndex);
            return false;
        }
        ...
        const callback = (source: string) => {
            ...
            this.triggerCardDeleteFromVertical(idx, currentData, source);
        };
        ...
        const connected = this.connectCollectorInfoButton(reviewBtnObj, debouncedCallback, 'CardDelete_' + cardIndex);
        if (connected) {
            this.reviewButtonHooked[cardIndex] = true;
            print('CollectionManager: [DELETE] Delete Item connected for ' + (data.brand_model || '?'));
            return true;
        }
        ...
        }, 'CardDeleteFallback_' + cardIndex, 0);
        if (fallbackConnected) {
            this.reviewButtonHooked[cardIndex] = true;
            print('CollectionManager: [DELETE] Button connected (fallback) for ' + (data.brand_model || '?'));
            return true;
        }

        print('CollectionManager: [DELETE] Could not hook Delete Item button for card #' + cardIndex
            + ' (' + (data.brand_model || '?') + ') — will retry next collection open');
        return false;
    }

    private findCollectorCombineButton(cardObj: SceneObject): SceneObject | null {
        const names = ['Review Button', 'Delete Clothes', 'Delete Item', 'Delete Item Button', 'Delete Button', 'Get More Info', 'Info Button'];
        for (let i = 0; i < names.length; i++) {
            const found = findChildByName(cardObj, names[i]);
            if (found) return found;
        }
        return this.findButtonByVisibleText(cardObj, ['delete item', 'delete', 'eliminar', 'supprimer'], 0);
    }

    private findCollectorDeleteHitTarget(cardObj: SceneObject): SceneObject | null {
        const visibleTargets = ['Delete Clothes', 'Delete Item', 'Delete Item Button', 'Delete Button'];
        for (let i = 0; i < visibleTargets.length; i++) {
            const found = findChildByName(cardObj, visibleTargets[i]);
            if (found && found.enabled) return found;
        }
        return this.findCollectorCombineButton(cardObj);
    }
    ...
    private tryTriggerCardMoreInfoFromPinch(): boolean {
        ...
        print('CollectionManager: [DELETE] Card delete fallback hit (pinchHitArea) for card #' + hit.cardIndex);
        this.triggerCardDeleteFromVertical(hit.cardIndex, hit.data, 'pinchHitArea');
        return true;
    }
    ...
    private setupCardInfoTapFallback(): void {
        const handleTap = (screenPoint: vec2, source: string) => {
            ...
            print('CollectionManager: [DELETE] Card delete fallback hit (' + source + ') for card #' + hit.cardIndex);
            this.triggerCardDeleteFromVertical(hit.cardIndex, hit.data, source);
        };
        ...
        tapEvent.bind((eventData: any) => {
            try {
                ...
                print('CollectionManager: [DELETE] tap fallback error: ' + e);
            }
        });
        ...
        touchEndEvent.bind((eventData: any) => {
            try {
                ...
                print('CollectionManager: [DELETE] touch fallback error: ' + e);
            }
        });
    }
    ...
    private findCardInfoButtonAtPoint(worldPoint: vec3): { cardIndex: number; data: SavedVehicleData; cardReviewText: Text | null } | null {
        ...
        for (let i = 0; i < this.collectionCardObjects.length; i++) {
            ...
            if (!card || !card.enabled) continue;
            if (i >= this.savedVehicles.length) continue;
            if (!this.canTriggerCardDelete(i)) continue;

            const button = this.findCollectorDeleteHitTarget(card);
            ...
            Base64.encodeTextureAsync(
                ...
                (b64: string) => {
                    try {
                        ...
                        print('CollectionManager: Saved compressed scan photo for ' + vehicleName
                            + ' — ' + (storedB64 ? storedB64.length : 0) + ' chars, persisted=' + persisted);
                    } catch (e) {
                        print('CollectionManager: Failed to save compressed scan photo: ' + e);
                    }
                ...
                () => { print('CollectionManager: Failed to encode compressed scan photo for storage'); },
    ...
    private saveCardImageBase64ToStorage(vehicleName: string, savedAt: number, base64: string): boolean {
        if (!base64 || base64.length === 0) return false;
        if (base64.length > this.SCAN_IMAGE_DIRECT_SAVE_MAX_CHARS) {
            print('CollectionManager: Scan photo for ' + vehicleName + ' is large — '
                + Math.round((base64.length * 0.75) / 1024) + 'KB; saving compressed copy');
            this.saveCompressedScanBase64ToStorage(vehicleName, savedAt, base64);
            return false;
        }
        try {
            const storageKey = this.IMAGE_KEY_PREFIX + savedAt.toString();
            const store = global.persistentStorageSystem.store;
            store.putString(storageKey, base64);
            const storedB64 = store.getString(storageKey);
            const persisted = !!storedB64 && storedB64.length === base64.length;
            print('CollectionManager: Saved real scan photo for ' + vehicleName
                + ' — ' + (storedB64 ? storedB64.length : 0) + '/' + base64.length + ' chars, verified=' + persisted);
            if (persisted) return true;

            try { store.putString(storageKey, ''); } catch (clearErr) { /* ignore */ }
            this.saveCompressedScanBase64ToStorage(vehicleName, savedAt, base64);
            return false;
        } catch (e) {
    ...
    private saveCompressedScanBase64ToStorage(vehicleName: string, savedAt: number, base64: string): void {
        if (!base64 || base64.length === 0) return;
        try {
            Base64.decodeTextureAsync(
                base64,
                (texture: Texture) => {
                    this.saveCardImageToStorage(vehicleName, savedAt, texture);
                },
                () => {
                    print('CollectionManager: Failed to decode scan photo for compressed storage');
                }
            );
        } catch (e) {
            print('CollectionManager: Compressed scan storage exception: ' + e);
        }
    }

    private clearStoredScanImage(savedAt: number, reason: string): void {
        if (!savedAt) return;
        try {
            const store = global.persistentStorageSystem.store;
            const storageKey = this.IMAGE_KEY_PREFIX + savedAt.toString();
            const existing = store.getString(storageKey);
            if (!existing || existing.length === 0) return;
            store.putString(storageKey, '');
            print('CollectionManager: [GARMENT-CUTOUT] Cleared duplicate scan photo after '
                + reason + ' — freed ' + Math.round((existing.length * 0.75) / 1024) + 'KB');
        } catch (e) {
            print('CollectionManager: [GARMENT-CUTOUT] Could not clear duplicate scan photo: ' + e);
        }
    }

    private pruneDuplicateScanImagesForGeneratedCards(): void {
        try {
            const store = global.persistentStorageSystem.store;
            for (let i = 0; i < this.savedVehicles.length; i++) {
                const savedAt = this.savedVehicles[i]?.savedAt;
                if (!savedAt) continue;
                const generated = store.getString(this.GARMENT_CUTOUT_KEY_PREFIX + savedAt.toString());
                if (!generated || generated.length === 0) continue;
                const scanKey = this.IMAGE_KEY_PREFIX + savedAt.toString();
                const scan = store.getString(scanKey);
                if (!scan || scan.length === 0) continue;
                store.putString(scanKey, '');
                print('CollectionManager: [GARMENT-CUTOUT] Pruned duplicate scan photo for card #' + (i + 1));
            }
        } catch (e) {
            print('CollectionManager: [GARMENT-CUTOUT] Duplicate scan prune failed: ' + e);
        }
    }
                ...
                (b64: string) => {
                    try {
                        ...
                        if (persisted) {
                            ...
                            this.clearStoredScanImage(savedAt, 'cutout save');
                        } else {
        ...
        try {
            ...
            Base64.decodeTextureAsync(
                ...
                (texture: Texture) => {
                    ...
                    this.clearStoredScanImage(savedAt, 'cutout load');
    ...
    private saveCollectionToStorage(): boolean {
        try {
            this.pruneDuplicateScanImagesForGeneratedCards();
            const store = global.persistentStorageSystem.store;
            ...
            const storedJson = store.getString(this.STORAGE_KEY);
            const persisted = !!storedJson && storedJson.length === json.length;
            print('CollectionManager: Saved ' + serializable.length + ' closet items — persisted=' + persisted);
            return persisted;
        } catch (e) {
    ...
    private async saveFullLookAsCards(): Promise<void> {
        ...
        if (capturedPhotoBase64.length > 0) {
            for (let i = 0; i < savedList.length; i++) {
                this.saveCardImageBase64ToStorage(savedList[i].item_name || savedList[i].brand_model, savedList[i].savedAt, capturedPhotoBase64);
            }
        }
