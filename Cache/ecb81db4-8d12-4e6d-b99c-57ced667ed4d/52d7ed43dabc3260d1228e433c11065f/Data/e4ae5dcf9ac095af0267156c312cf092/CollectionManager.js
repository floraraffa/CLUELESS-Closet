...
let CollectionManager = (() => {
    ...
    var CollectionManager = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.SCAN_IMAGE_DIRECT_SAVE_MAX_CHARS = 650000;
        ...
        openDeleteConfirmForCard(targetIdx, source) {
        ...
        populateCollectorCard(cardObj, data) {
            ...
            this.setButtonLabel(cardObj, 'Review Button', (0, Localization_1.t)('delete_item_button'));
        ...
        findCardInfoButtonAtPoint(worldPoint) {
            ...
            for (let i = 0; i < this.collectionCardObjects.length; i++) {
                ...
                if (i >= this.savedVehicles.length)
                    continue;
                if (!this.canTriggerCardDelete(i))
                    continue;
        ...
        saveCardImageToStorage(vehicleName, savedAt, texture, serial) {
            try {
                ...
                }, () => { print('CollectionManager: Failed to encode compressed scan photo for storage'); }, CompressionQuality.MaximumCompression, EncodingType.Jpg);
            }
            catch (e) { /* ignore */ }
        }
        ...
        saveCompressedScanBase64ToStorage(vehicleName, savedAt, base64) {
            if (!base64 || base64.length === 0)
                return;
            try {
        ...
        clearStoredScanImage(savedAt, reason) {
            if (!savedAt)
                return;
            try {
                const store = global.persistentStorageSystem.store;
        ...
        pruneDuplicateScanImagesForGeneratedCards() {
            try {
                const store = global.persistentStorageSystem.store;
                for (let i = 0; i < this.savedVehicles.length; i++) {
                    const savedAt = this.savedVehicles[i]?.savedAt;
        ...
        saveGarmentCutoutTextureToStorage(savedAt, texture, onComplete) {
            ...
            try {
                ...
                Base64.encodeTextureAsync(texture, (b64) => {
                    ...
                    if (onComplete)
                        onComplete(false);
                }, CompressionQuality.MaximumCompression, EncodingType.Jpg);
            }
            catch (e) {
