...
let CollectionManager = (() => {
    ...
    var CollectionManager = _classThis = class extends _classSuper {
        constructor() {
            ...
            // =====================================================================
            this.shareCollectionButton = this.shareCollectionButton;
            this.confirmShareContainer = this.confirmShareContainer;
            this.confirmShareYesButton = this.confirmShareYesButton;
            this.confirmShareCancelButton = this.confirmShareCancelButton;
            this.shareConfirmationText = this.shareConfirmationText;
            ...
            this.onCarouselVisibilityChanged = null;
            ...
            this.TRADE_HISTORY_KEY = 'dgns_trade_history';
            ...
            this.outfitSlotToSavedIndex = [-1, -1, -1, -1];
            ...
            this.onCloudSyncTradeHistory = null;
            ...
            this.shareButtonConnected = false;
            this.confirmShareConnected = false;
        ...
        onShareButtonPressed() {
            ...
            if (this.savedVehicles.length === 0) {
                if (this.onShowDescription)
                    this.onShowDescription((0, Localization_1.t)('no_vehicles_share'));
                return;
            }
            if (this.confirmShareContainer) {
                this.showContainer(this.confirmShareContainer, () => this.onConfirmShareCancel(), 'ConfirmShareClose');
            }
            if (this.shareConfirmationText)
                this.shareConfirmationText.text = (0, Localization_1.t)('share_confirm');
            if (this.onShowDescription)
                this.onShowDescription((0, Localization_1.t)('share_collection_q'));
        }
        onConfirmShareYes() {
            if (this.isSharing)
                return;
            this.isSharing = true;
            if (this.shareConfirmationText)
                this.shareConfirmationText.text = (0, Localization_1.t)('sharing');
            if (this.onShowDescription)
                this.onShowDescription((0, Localization_1.t)('sharing_collection'));
            if (this.onShareCollection) {
                this.onShareCollection((url) => {
                    this.isSharing = false;
                    if (this.shareConfirmationText)
                        this.shareConfirmationText.text = (0, Localization_1.t)('share_url') + url;
                    if (this.onShowDescription)
                        this.onShowDescription((0, Localization_1.t)('collection_shared'));
                ...
                this.isSharing = false;
                if (this.shareConfirmationText)
                    this.shareConfirmationText.text = (0, Localization_1.t)('share_unavailable');
                if (this.onShowDescription)
                    this.onShowDescription((0, Localization_1.t)('share_unavail_short'));
                const hideDelay = this.createEvent('DelayedCallbackEvent');
                hideDelay.reset(4.0);
                hideDelay.bind(() => {
                    if (this.confirmShareContainer)
        ...
        onConfirmShareCancel() {
            this.isSharing = false;
            if (this.shareConfirmationText)
                this.shareConfirmationText.text = (0, Localization_1.t)('collection_not_shared');
            if (this.onShowDescription)
                this.onShowDescription((0, Localization_1.t)('collection_not_shared'));
            const hideDelay = this.createEvent('DelayedCallbackEvent');
            hideDelay.reset(3.0);
            hideDelay.bind(() => {
                if (this.confirmShareContainer)
        ...
        translateStaticTexts() {
            ...
            if (this.confirmShareContainer)
                this.walkAndTranslate(this.confirmShareContainer);
            if (this.confirmResetProfileContainer)
            ...
            try {
                ...
                if (textComp) {
                    if (name === 'Yes') {
                        ...
                        textComp.text = (0, Localization_1.t)('share_question');
                    }
        ...
        async onSaveButtonPressed() {
            ...
            try {
                ...
                const savedData = {
                    ...
                    rarity_label: this.lastVehicleData.rarity_label || (0, VehicleTypes_1.getRarityLabel)(this.lastVehicleData.rarity || 2),
                    scene: this.lastVehicleData.scene || '',
                    savedAt: scanTimestamp,
                    imageGenerated: false,
                    serial: (0, VehicleTypes_1.generateSerial)(),
                    dateScanned: (0, VehicleTypes_1.formatScanDate)(scanTimestamp),
        ...
        showCollection() {
            ...
            for (let i = 0; i < this.collectionCardObjects.length; i++) {
                ...
                if (state === this.STATE_PLACED_IN_WORLD)
                    continue;
                card.enabled = true;
                (0, VehicleTypes_1.enableAllDescendants)(card);
                // Hide card images whose texture hasn't loaded yet (BUG 1 fix)
                if (!this.cardImageReady[i]) {
                    const cardImageObj = (0, VehicleTypes_1.findChildByName)(card, 'Card Image');
                    if (cardImageObj)
        ...
        addReceivedCard(card, imageB64) {
            ...
            for (let i = 0; i < this.savedVehicles.length; i++) {
                if (this.savedVehicles[i] && this.savedVehicles[i].serial === card.serial) {
                    ...
                    if (this.onShowDescription)
                        this.onShowDescription((0, Localization_1.t)('card_duplicate'));
                    return;
                }
            }
            const now = Date.now();
            ...
            const savedData = {
                ...
                scan_context: (0, VehicleTypes_1.normalizeScanContext)(card.scan_context),
                type: card.type,
                category: card.type,
                year: card.year,
                ...
                serial: card.serial,
                dateScanned: card.dateScanned || (0, VehicleTypes_1.formatScanDate)(now),
            ...
            if (this.verticalCardPrefab) {
                try {
                    ...
                    if (cardObj) {
                        ...
                        if (imageB64 && imageB64.length > 0) {
                            Base64.decodeTextureAsync(imageB64, (texture) => { this.applyCardImage(cardObj, texture); }, () => { print('CollectionManager: Failed to decode traded card image'); });
                        }
                    }
                }
                catch (e) {
            ...
            if (this.onShowDescription)
                this.onShowDescription((0, Localization_1.tf)('card_added', { name: card.brand_model }));
            if (this.onHideDescriptionAfterDelay)
                this.onHideDescriptionAfterDelay(3.0);
        }
        /**
        ...
        removeCardBySerial(serial) {
            ...
            if (this.onShowDescription)
                this.onShowDescription((0, Localization_1.tf)('card_given_away', { name: name }));
            if (this.onHideDescriptionAfterDelay)
                this.onHideDescriptionAfterDelay(3.0);
        }
        // =====================================================================
        ...
        addTradeRecord(entry) {
            this.tradeHistory.push(entry);
            this.saveTradeHistory();
            if (this.onCloudSyncTradeHistory)
                this.onCloudSyncTradeHistory(entry);
        ...
        loadTradeHistory() {
            try {
                ...
                if (json && json.length > 2) {
                    this.tradeHistory = JSON.parse(json);
        ...
        populateCollectorCard(cardObj, data) {
            ...
            set('Car Type', (0, VehicleTypes_1.formatCarType)(data.category || data.type));
            const metaParts = [];
            ...
            if (rarityObj) {
                ...
                if (tc) {
                    ...
                    tc.text = (0, VehicleTypes_1.formatRarityText)(rarity, data.rarity_label || (0, VehicleTypes_1.getRarityLabel)(rarity));
                }
            }
            // Stat bars
            this.updateStatBar((0, VehicleTypes_1.findChildByName)(cardObj, 'Top Speed Bar'), data.top_speed);
            this.updateStatBar((0, VehicleTypes_1.findChildByName)(cardObj, 'Acceleration Bar'), data.acceleration);
            this.updateStatBar((0, VehicleTypes_1.findChildByName)(cardObj, 'Braking Bar'), data.braking);
            this.updateStatBar((0, VehicleTypes_1.findChildByName)(cardObj, 'Traction Bar'), data.traction);
            this.updateStatBar((0, VehicleTypes_1.findChildByName)(cardObj, 'Comfort Bar'), data.comfort);
            ...
            this.setButtonLabel(cardObj, 'Review Button', (0, Localization_1.t)('combine_look_button'));
        ...
        setButtonLabel(cardObj, buttonName, text) {
            const buttonObj = (0, VehicleTypes_1.findChildByName)(cardObj, buttonName);
            if (!buttonObj)
                return;
         ...
         */
        canTriggerCombineLook(cardIndex) {
        ...
        tryHookReviewButton(cardObj, cardIndex) {
            ...
            const carReviewObj = (0, VehicleTypes_1.findChildByName)(cardObj, 'Car Review');
            let carReviewText = null;
            if (carReviewObj) {
                carReviewText = carReviewObj.getComponent('Component.Text');
            }
        ...
        findCollectorCombineButton(cardObj) {
            ...
            for (let i = 0; i < names.length; i++) {
                const found = (0, VehicleTypes_1.findChildByName)(cardObj, names[i]);
                if (found)
                    return found;
            }
        ...
        updateGarmentPlaceholderForSavedItem(data, savedIndex, capturedBase64, fallbackTexture, sourceCardObj) {
            ...
            const templateImageObj = sourceCardObj ? (0, VehicleTypes_1.findChildByName)(sourceCardObj, 'Card Image') : null;
        ...
        async generateGarmentCutoutTexture(data, capturedBase64) {
            let imageBytes;
            try {
        ...
        restoreOutfitSlotDefault(slotIndex) {
            ...
            if (defaultTexture)
                this.applyTextureToSceneObject(slotObj, defaultTexture);
        ...
        copyCardImageTextureToSlot(cardObj, slotObj) {
            const cardImageObj = cardObj ? (0, VehicleTypes_1.findChildByName)(cardObj, 'Card Image') : null;
        ...
        getOutfitCandidateScore(data, slotIndex) {
            ...
            score += (0, VehicleTypes_1.clampStat)(data.top_speed || 3);
            score += (0, VehicleTypes_1.clampStat)(data.acceleration || 3);
            score += (0, VehicleTypes_1.clampStat)(data.braking || 3);
            score += (0, VehicleTypes_1.clampStat)(data.traction || 3);
            score += (0, VehicleTypes_1.clampStat)(data.comfort || 3);
        ...
        getPreferredOutfitSlotForItem(data) {
            ...
            if (this.isLookBottomLike(text))
                return 2;
            if (this.isLookTopLike(text) || this.isLookOuterwearLike(text) || this.isLookDressLike(text) || this.isLookFullOutfit(text))
                return 1;
            return -1;
        }
        ...
        getOutfitSlotAliases(slotIndex) {
            ...
            if (slotIndex === 1)
                return ['photocard TOP', 'PhotoCard TOP', 'Photocard TOP', 'Photo Card TOP', 'photocard TORSO', 'Garment TOP', 'TOP', 'Torso'];
            if (slotIndex === 2)
        ...
        saveCollectionToStorage() {
            try {
                ...
                const serializable = activeVehicles.map(v => ({
                    ...
                    rarity: v.rarity || 2, rarity_label: v.rarity_label || (0, VehicleTypes_1.getRarityLabel)(v.rarity || 2),
                    color: v.color || '',
                    material: v.material || '',
                    pattern: v.pattern || '',
                    fit: v.fit || '',
            ...
            try {
                ...
                for (let i = 0; i < this.savedVehicles.length; i++) {
                    ...
                    if (!v.serial || v.serial.length === 0) {
                        v.serial = (0, VehicleTypes_1.generateSerial)();
                        needsResave = true;
                    ...
                    if (!v.dateScanned || v.dateScanned.length === 0) {
                        ...
                        v.dateScanned = v.savedAt ? (0, VehicleTypes_1.formatScanDate)(v.savedAt) : '';
                        needsResave = true;
                    }
                    ...
                    if (!v.scan_context || v.scan_context.length === 0) {
                        ...
                        v.scan_context = (0, VehicleTypes_1.normalizeScanContext)(v.scan_context);
                    }
