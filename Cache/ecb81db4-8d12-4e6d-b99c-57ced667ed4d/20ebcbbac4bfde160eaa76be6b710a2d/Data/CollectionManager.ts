...
export class CollectionManager extends BaseScriptComponent {
    ...
    private tryHookReviewButton(cardObj: SceneObject, cardIndex: number): boolean {
        ...
        // Find the "Car Review" Text component on the card (for the AI garment note)
        const carReviewObj = findChildByName(cardObj, 'Car Review');
        let carReviewText: Text | null = null;
        if (carReviewObj) {
            carReviewText = this.findFirstTextInSubtree(carReviewObj, 0);
        }
        ...
        const callback = (source: string) => {
            ...
            this.triggerCardMoreInfo(idx, currentData, carReviewText, source);
        };
        ...
        if (this.onConnectButton) {
            ...
            if (connected) {
                ...
                print('CollectionManager: [INFO] Button connected for ' + (data.brand_model || '?'));
                return true;
            }
        }
