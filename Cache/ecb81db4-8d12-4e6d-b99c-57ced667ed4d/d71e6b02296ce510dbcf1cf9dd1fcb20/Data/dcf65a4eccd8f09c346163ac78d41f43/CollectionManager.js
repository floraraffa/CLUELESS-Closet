...
let CollectionManager = (() => {
    ...
    var CollectionManager = _classThis = class extends _classSuper {
        ...
        applyTrustColorToText(textComp) {
            ...
            try {
                ...
                if (match) {
                    ...
                    const hex = (0, ClosetTypes_1.getTrustColor)(score);
        ...
        async saveFullLookAsCards() {
            ...
            // 1) Build all cards up front (hidden), each placed in its grid slot
            for (let i = 0; i < items.length; i++) {
            ...
            this.saveCollectionToStorage();
            // 2) Generate each garment cut-out sequentially; reveal each card as it finishes
            for (let i = 0; i < cardObjs.length; i++) {
                const name = savedList[i].item_name || savedList[i].brand_model;
                if (showStatus)
                    showStatus((0, Localization_1.tf)('outfit_card_progress', { index: i + 1, total: cardObjs.length, name: name }));
                // Show the scan photo immediately, then reveal the card into its grid slot
                const fullTex = await this.decodeBase64Texture(capturedPhotoBase64);
                if (fullTex)
                    this.applyCardImage(cardObjs[i], fullTex);
                this.revealOutfitCard(cardObjs[i], this.computeOutfitGridSlot(i, cardObjs.length, headData));
                try {
                    const cutout = await this.generateGarmentCutoutTexture(savedList[i], capturedPhotoBase64, true);
                    this.applyCardImage(cardObjs[i], cutout);
                    this.saveGarmentCutoutTextureToStorage(savedList[i].savedAt, cutout, (saved, b64) => {
                        const idx = this.findSavedVehicleIndexBySerial(savedList[i].serial);
                        if (idx < 0)
                            return;
                        this.savedVehicles[idx].imageGenerated = saved;
                        this.saveCollectionToStorage();
                        if (saved && b64 && this.onCloudUploadImage)
                            this.onCloudUploadImage(savedList[i].serial, b64);
                    });
                }
                catch (e) {
                    print('CollectionManager: [OUTFIT] Cutout failed for ' + name + ' — keeping scan photo: ' + e);
                }
            }
            if (showStatus)
            ...
            if (this.onCardGenerationSuccess)
                this.onCardGenerationSuccess();
            // 3) After a short beat with the full grid shown, fly all cards into the collection
