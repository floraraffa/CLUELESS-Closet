...
export class ClosetClubScanner extends BaseScriptComponent {
    ...
    @input
    @allowUndefined
    @hint('OnboardingManager script — first-launch guided tutorial (optional)')
    ...
    private wireModules(): void {
        ...
        if (this.vehicleCardUI) {
            ...
            this.vehicleCardUI.onScanStarted = () => {
                ...
                if (this.onboardingManager) this.onboardingManager.notifyScanStarted();
            };
            this.vehicleCardUI.onScanEnded = () => {
                // Scanning finished — idle mode will restart when scan interface re-appears
                if (this.onboardingManager) this.onboardingManager.notifyScanEnded();
            };
        ...
        if (this.vehicleNarrator) {
            ...
            this.vehicleNarrator.onGetStyleContext = (): string => {
                return this.collectionManager ? this.collectionManager.buildStyleContext() : '';
            };
        ...
        if (this.collectionManager) {
            this.collectionManager.onCollectionOpened = () => {
                // Opening the closet closes the scan interface so they don't overlap.
                this.hideScanInterface();
                if (this.onboardingManager) this.onboardingManager.notifyCollectionOpened();
            };
            ...
            this.collectionManager.onStatusBanner = (message: string, kind: string) => {
                this.showStatusBanner(message, kind);
            };
            ...
            this.collectionManager.onCardSaved = (data: SavedVehicleData) => {
                ...
                if (this.onboardingManager) this.onboardingManager.notifyCardSaved();
            };
        ...
        if (this.welcomeManager) {
            ...
            this.welcomeManager.onSoloModeSelected = () => {
                ...
                this.showScanInterface();
                if (this.onboardingManager) this.onboardingManager.notifyLanguageAccepted();
            };
        ...
        // --- OnboardingManager wiring ---
        if (this.onboardingManager) {
            this.onboardingManager.onConnectButton = (obj: SceneObject, cb: () => void, name: string): boolean => {
                if (this.vehicleCardUI) return this.vehicleCardUI.connectButton(obj, cb, name);
                return false;
            };
        }
    ...
    private async onScanButtonPressed(): Promise<void> {
        ...
        // The scan needs internet (GPT Vision). Tell the user clearly if offline.
        try {
            if (global.deviceInfoSystem && !global.deviceInfoSystem.isInternetAvailable()) {
                print('ClosetClubScanner: Scan blocked — no internet');
                this.showStatusBanner(t('no_internet'), 'warning');
                if (this.vehicleNarrator) this.vehicleNarrator.showStatusError(t('no_internet'), 6.0);
                return;
            }
        } catch (e) { /* can't tell — let the scan try */ }
    ...
    /**
     * Central status banner — reuses the camera-attached "welcome back" text
     * element (xpDisplay) to surface app events to the user: card added, image
     * generation failed, no internet, etc. Color-coded by kind.
     */
    private showStatusBanner(message: string, kind: string): void {
        if (!message || message.length === 0) return;
        let r = 1, g = 1, b = 1;
        let duration = 4.0;
        if (kind === 'success') { r = 0.55; g = 0.95; b = 0.6; duration = 3.5; }
        else if (kind === 'error') { r = 1.0; g = 0.5; b = 0.45; duration = 6.0; }
        else if (kind === 'warning') { r = 1.0; g = 0.82; b = 0.4; duration = 6.0; }
        this.showOnXPDisplay(message, r, g, b, duration);
    }
