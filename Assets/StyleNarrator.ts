...
import { VehicleData } from './ClosetTypes';
import { t, tf, getIdlePhrases } from './Localization';

@component
export class StyleNarrator extends BaseScriptComponent {
    ...
    /** Returns the current styling context (season/city/time + wardrobe richness) for outfit feedback. */
    onGetStyleContext: (() => string) | null = null;
    ...
    private scrollTextTarget: Text | null = null;
    private speechTextTarget: Text | null = null;
    ...
    private forceFreshReviewAI: boolean = false;
    private reviewRequestId: number = 0;
    ...
    private cancelActiveReviewForNewCard(): void {
        ...
        try {
            if (this.ttsAudioPlayer) this.ttsAudioPlayer.stop(true);
        } catch (e) { /* ignore */ }
        this.speechTextTarget = null;
        this.scrollTextTarget = null;
        ...
        if (this.isReviewFromCard) this.restoreCardReviewText();
    }
    ...
    async triggerOutfitFeedback(outfitItems: VehicleData[], slotLabels?: string[], outfitText?: Text | null, percentText?: Text | null, matchPercent?: number): Promise<void> {
        ...
        this.isFetchingInfo = true;
        if (this.onReviewGenerationStarted) this.onReviewGenerationStarted();
        if (!outfitText) this.showAnimatedDescriptionText(t('outfit_feedback_loading'));
    ...
    private async speakDescriptionToExternalText(text: string, targetText: Text | null | undefined): Promise<void> {
        if (!targetText) {
            await this.speakDescription(text);
            return;
        }
        this.speechTextTarget = targetText;
        await this.speakDescription(text);
    }
    ...
    // =====================================================================
    // ONBOARDING NARRATION (generic "speak this text" entry point)
    // =====================================================================
    // Dedicated, self-contained TTS path for the OnboardingManager tutorial.
    // Kept separate from speakDescription() to avoid the review-card coupling
    // (carDescriptionText / isReviewFromCard / speechTextTarget). Reuses the
    // same AudioComponent + onTTSPlaybackStarted/Ended callbacks (so music
    // ducking still works automatically), and invokes onDone when the line
    // finishes (or after a read-time delay if TTS is unavailable/offline).
    private onboardingRequestId: number = 0;

    /**
     * Speak an arbitrary (already-localized) line for the onboarding tutorial.
     * @param text       the line to narrate
     * @param subtitle   optional Text component to mirror the line on-screen
     * @param onDone      called once narration (or its fallback delay) completes
     */
    async speakOnboarding(text: string, subtitle?: Text | null, onDone?: (() => void) | null): Promise<void> {
        const reqId = ++this.onboardingRequestId;
        if (subtitle) subtitle.text = text;

        const fallbackDelay = Math.max(2.0, Math.min(12.0, text.length * 0.06));
        const finishViaDelay = () => {
            const ev = this.createEvent('DelayedCallbackEvent');
            ev.bind(() => {
                if (reqId !== this.onboardingRequestId) return;
                if (onDone) onDone();
            });
            ev.reset(fallbackDelay);
        };
        ...
        try {
            ...
            print('StyleNarrator: onboarding TTS error: ' + error);
            if (this.ttsIsPlaying && this.onTTSPlaybackEnded) this.onTTSPlaybackEnded();
            this.ttsIsPlaying = false;
            if (reqId !== this.onboardingRequestId) return;
            // Offline / TTS failed: still advance the tutorial after a read delay.
            finishViaDelay();
        }
    }

    /** Cancel any in-flight onboarding narration (used when skipping). */
    stopOnboarding(): void {
    ...
    private async fetchVehicleDescription(vehicleName: string, vehicleType: string, brand: string, compactForCard: boolean = false): Promise<string> {
...
${langInstruction}`;
    ...
    private async fetchOutfitFeedback(outfitItems: VehicleData[], slotLabels: string[], matchPercent?: number): Promise<string> {
        ...
        const systemPrompt = `You are the resident stylist of an AI closet app called Closet Club — and you are anything but boring.

PERSONA:
- You are a flamboyant, theatrical, eccentric haute-couture maestro — a fashion-week legend in the spirit of Karl Lagerfeld. You worship great fashion and hold impossibly high standards.
- You are witty, dramatic and quotable, a little judgmental — but never cruel. Your shade is playful; your praise is euphoric.
- When the closet barely gives you anything to work with (e.g. only t-shirts and shorts for a cold winter morning), sigh theatrically and own it — something like: "ahh... I do what I can with what I have, do not expect a miracle."
- When the closet is full of beautiful pieces, be giddy, generous and delighted.
- Weave the season, the city, the day of the week and the time of day into your reasoning, and justify the look for that exact occasion.
...
${langInstruction}`;
        ...
        const userPrompt = 'Composed outfit slots:\n' + outfitLines.join('\n')
            + percentLine
            + (styleContext ? '\n\nSetting:\n' + styleContext : '')
            + '\n\nGive text and voice-ready feedback, fully in character. The first phrase must clearly say whether this outfit matches or does not match, and your reasoning should fit the season, place, day and time of day.';
    ...
    private async speakDescription(text: string): Promise<void> {
        const requestId = this.reviewRequestId;
        const externalTarget = this.speechTextTarget;

        if (!this.ttsAudioPlayer) {
            this.ttsIsPlaying = false;
            this.startTextScroll(text);
            if (externalTarget) {
                const doneEv = this.createEvent('DelayedCallbackEvent');
                doneEv.bind(() => {
                    if (requestId !== this.reviewRequestId) return;
                    this.setExternalText(externalTarget, text);
                    if (this.speechTextTarget === externalTarget) this.speechTextTarget = null;
                });
                doneEv.reset(15.0);
            } else if (this.isReviewFromCard) {
    ...
    private startTextScroll(fullText: string): void {
        this.stopTextScroll();
        this.stopDotsAnimation();
        const targetText = this.speechTextTarget || this.carDescriptionText;
        this.scrollTextTarget = targetText;
        if (!targetText) return;

        const descObj = targetText.getSceneObject();
        if (descObj) descObj.enabled = true;
    ...
    private updateScrollDisplay(): void {
        const targetText = this.scrollTextTarget || this.carDescriptionText;
        if (!targetText) return;
        const start = Math.max(0, this.scrollIndex - 1);
        const end = Math.min(this.scrollSentences.length, this.scrollIndex + 2);
        targetText.text = this.scrollSentences.slice(start, end).join(' ');
    }

    private scheduleNextScroll(): void {
        if (this.scrollIndex >= this.scrollSentences.length - 1) {
            if (!this.ttsIsPlaying && !this.speechTextTarget) this.hideDescriptionAfterDelay(3.0);
            return;
        }
