...
export class ClothingScanner extends BaseScriptComponent {
    ...
    // ---- Fixed "in front of user" placement (Closet Club) ----
    @input
    @hint('Distance the result card sits in front of the user, along gaze (cm). Recommended: 60')
    cardDistanceFromUser: number = 60;

    @input
    @hint('Fixed world scale of the result card (no depth scaling). Tweak in Preview. Try 1.0 - 3.0')
    cardFixedScale: number = 1.5;
    ...
    private readonly DEFAULT_CARD_DISTANCE: number = 60;
    private readonly DEFAULT_FIXED_SCALE: number = 1.5;
    ...
    private async captureImage(): Promise<string> {
        ...
        if (stillTexture) {
            ...
            // The RSG image-edit module sends the file as image/png, so encode PNG here.
            try {
                const editBase64 = await this.textureToBase64ForEdit(stillTexture);
                this.lastCapturedBase64Edit = editBase64 && editBase64.length > 0 ? editBase64 : base64;
                print('ClothingScanner: Edit JPG (' + Math.round((this.lastCapturedBase64Edit.length * 0.75) / 1024) + ' KB)');
            } catch (editErr) {
    ...
    private textureToBase64ForEdit(texture: Texture): Promise<string> {
        return new Promise<string>((resolve, reject) => {
            try {
                // Encode as JPG, NOT PNG. PNG is lossless, so a full-res camera
                // still becomes ~12 MB even at LowQuality — large enough that the
                // Remote Service Gateway rejects the image-edit upload outright
                // (empty error in ~1s) AND Base64.decodeTextureAsync fails, so the
                // card showed no image at all. gpt-image-1 image-edit accepts JPG
                // (it sniffs the actual bytes).
                //
                // Use IntermediateQuality (not HighQuality): a smaller image (a)
                // decodes back to a texture reliably for the fallback card photo,
                // and (b) lets gpt-image-1 process faster, so the edit is far less
                // likely to hit the gateway's ~30s proxy timeout ("internal error").
                Base64.encodeTextureAsync(
                    texture,
                    (base64String: string) => resolve(base64String),
...
- Only if no wearable wardrobe item is visible at all (an empty scene or an unrelated object), return clothing_found false and brand_model "NO_CLOTHING".`;
    ...
    private positionCardAboveVehicle(): void {
        ...
        // Closet Club: the result card is a UI panel that should appear at a
        // fixed, comfortable size and distance IN FRONT OF THE USER (not anchored
        // to the scanned garment, and NOT depth-scaled). Orientation is handled by
        // the Billboard component on the card's Frame — we do not set rotation here.
        const distance = this.cardDistanceFromUser > 0 ? this.cardDistanceFromUser : this.DEFAULT_CARD_DISTANCE;
        const fixedScale = this.cardFixedScale > 0 ? this.cardFixedScale : this.DEFAULT_FIXED_SCALE;

        // Fixed size — no depth-based scaling.
        cardTransform.setWorldScale(new vec3(fixedScale, fixedScale, fixedScale));

        // No depth data -> fallback straight ahead in world space.
        if (!this.latestDepthData) {
            cardTransform.setWorldPosition(new vec3(0, 0, -distance));
            return;
        }
        ...
        try {
            ...
            // Gaze direction: from the eye toward whatever is at screen center.
            // Derived from the unprojected garment point, so it is independent of
            // device-space axis-sign conventions. Falls back to device forward (-Z)
            // when depth at the center is invalid.
            ...
            const depthValue = this.latestDepthData.depthFrame[pixelX + pixelY * resolution.x];

            let gazeDir: vec3;
            if (depthValue && depthValue > 50 && depthValue <= 2000) {
                const point3D = depthCamera.unproject(normalizedCoord, depthValue);
                const garmentWorld = worldFromDevice.multiplyPoint(point3D);
                gazeDir = garmentWorld.sub(camPos);
            } else {
                gazeDir = new vec3(-worldFromDevice.column2.x, -worldFromDevice.column2.y, -worldFromDevice.column2.z);
            }
            if (gazeDir.length < 0.01) gazeDir = new vec3(0, 0, -1);
            gazeDir = gazeDir.normalize();

            // Place the card a fixed distance in front of the user, along the gaze.
            const cardPosition = new vec3(
                camPos.x + gazeDir.x * distance,
                camPos.y + gazeDir.y * distance,
                camPos.z + gazeDir.z * distance
            );
            cardTransform.setWorldPosition(cardPosition);

            print('ClothingScanner: Card placed ' + distance.toFixed(0) + 'cm in front of user, scale=' + fixedScale.toFixed(2));
        } catch (error) {
